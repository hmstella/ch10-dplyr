...
biggest_decrease <- data %>%
  filter(Pulitzer.Prize.Winners.and.Finalists..2004.2014 > 4) %>%
...
# which pucblication has the highest ratio of 
# winners in 2004-2014 of the total number of winners?
new_data <- data %>%
  mutate(ratio = round((Pulitzer.Prize.Winners.and.Finalists..2004.2014 / 
           Pulitzer.Prize.Winners.and.Finalists..1990.2014) * 100, 1)) %>%
  filter(ratio != 100.0)
highest_ratio <- new_data %>%
